...
public class tcss543 {
	...
	public static void main(String args[]) throws IOException{
		...
		//runs the Scaling ford fulkerson algorithm
				runAlgo(filenameInput, 
						(String filename) -> { ScalingMaxFlow sc = new ScalingMaxFlow(); 
												try {
													return sc.run(filename);
												} catch (Exception e) {
													return -1;
												}}, 
						"Scaling Ford Fulkerson Algorithm");
		
		//runs the preflow-push algorithm
		runAlgo(filenameInput, 
				(String filename) -> { PreFlowPushAlgo pa = new PreFlowPushAlgo(); 
										try {
											return pa.runner(filename);
										} catch (Exception e) {
											return -1;
										}}, 
